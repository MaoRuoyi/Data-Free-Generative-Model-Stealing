...
for epoch in range(train_epoch):
    ...
    for step, (img, target) in enumerate(dataloader):
        img = img.to(device)
        target = onehot(target).to(device)
        size = img.size(0)
    ...
    with torch.no_grad():
        ...
        if (epoch+1)%check_epoch == 0:
            ...
            if epoch > 20:
                fid_mnist = fid_score.calculate_fid_given_paths(['./g_img/epoch_{}/g_all'.format(epoch+1),
                                                            '../o_img_6000'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
                ...
                print('FID to mnist: '+str(fid_mnist))
                fid_victim = fid_score.calculate_fid_given_paths(['./g_img/epoch_{}/g_all'.format(epoch+1),
                                                            dataset_path+'g_all'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
                ...
                print('FID to victim: '+str(fid_victim))
...
torch.save(G, "MNIST_cGAN_results/generator_param.pth")#torch.save()函数用于保存模型的状态字典，将模型的每一层映射到其对应的张量参数（权重和偏差）作为键值对，D.state_dict()返回每个模型的当前状态字典。状态字典包含模型的所有学习参数，可用于恢复训练或稍后加载模型以进行推理
...
show_fid_mnist_hist(fid_hist, save=True, path='MNIST_cGAN_results/MNIST_cGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='MNIST_cGAN_results/MNIST_cGAN_FID_hist_victim.png')
...
with open('MNIST_cGAN_results/MNIST_cGAN_FID_log.txt','w+') as l_w:
    l_w.write(fid_log)
l_w.close()
