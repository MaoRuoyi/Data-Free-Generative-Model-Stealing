...
if not os.path.isdir('MNIST_cDCGAN_results_s'):
    os.mkdir('MNIST_cDCGAN_results_s')
if not os.path.isdir('MNIST_cDCGAN_results_s/Fixed_results'):
    os.mkdir('MNIST_cDCGAN_results_s/Fixed_results')
if not os.path.isdir('MNIST_cDCGAN_results_s/Backup'):
    os.mkdir('MNIST_cDCGAN_results_s/Backup')
if not os.path.isdir('dc_img_s'):
    os.mkdir('dc_img_s')
...
dataset=MyDataset(txt_path = "./Dataset_Clean.txt", transform=img_transform)
...
for epoch in range(train_epoch):
    ...
    with torch.no_grad():
        ...
        fixed_p = 'MNIST_cDCGAN_results_s/Fixed_results/MNIST_cDCGAN_' + str(epoch + 1) + '.png'
        ...
        if (epoch+1)%check_epoch == 0:
            torch.save(G, 'MNIST_cDCGAN_results_s/Backup/generator_epoch_{}.pth'.format(epoch+1))
            torch.save(D, 'MNIST_cDCGAN_results_s/Backup/discriminator_epoch_{}.pth'.format(epoch+1))
            if not os.path.exists('./dc_img_s/epoch_'+str(epoch+1)):
                os.mkdir('./dc_img_s/epoch_'+str(epoch+1))
            for label in range(10):
                ...
                for g in range(picture_num_generate):
                    if not os.path.exists('./dc_img_s/epoch_'+str(epoch+1)+'/'+str(label)):
                        os.mkdir('./dc_img_s/epoch_'+str(epoch+1)+'/'+str(label))
                    if not os.path.exists('./dc_img_s/epoch_'+str(epoch+1)+'/g_all'):
                        os.mkdir('./dc_img_s/epoch_'+str(epoch+1)+'/g_all')
                    ...
                    save_image(prediction_img.clamp(0,1), './dc_img_s/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), nrow=1, format='png')
                    if g < picture_num_calculate:
                        copyfile('./dc_img_s/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), './dc_img_s/epoch_{}/g_all/fake_images_{}_{}.png'.format(epoch+1,label,g+1))
            print("Image of epoch "+str(epoch+1)+" saved.")

            #Calculate fid scores to MNIST and to victim output and save in dictionary
            fid_mnist = fid_score.calculate_fid_given_paths(['./dc_img_s/epoch_{}/g_all'.format(epoch+1),
                                                            '../o_img_6000'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
            ...
            fid_victim = fid_score.calculate_fid_given_paths(['./dc_img_s/epoch_{}/g_all'.format(epoch+1),
                                                            dataset_path+'g_all'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
...
torch.save(G, "MNIST_cDCGAN_results_s/generator_param.pth")
torch.save(D, "MNIST_cDCGAN_results_s/discriminator_param.pth")

#Plot result figures
show_train_hist(train_hist, save=True, path='MNIST_cDCGAN_results_s/MNIST_cDCGAN_train_hist.png')
show_fid_mnist_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_s/MNIST_cDCGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_s/MNIST_cDCGAN_FID_hist_victim.png')
...
with open('MNIST_cDCGAN_results_s/MNIST_cDCGAN_FID_log.txt','w+') as l_w:
    l_w.write(fid_log)
l_w.close()
