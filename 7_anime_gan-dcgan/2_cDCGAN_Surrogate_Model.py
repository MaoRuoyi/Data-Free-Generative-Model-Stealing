...
dataset_path = './g_img/epoch_960/g_all'
...
#Define loading of self-built dataset from the folder storing best performance victim output
...
d_optimizer = torch.optim.Adam(D.parameters(), lr=0.000005, betas=(0.5, 0.999))
g_optimizer = torch.optim.Adam(G.parameters(), lr=0.00005, betas=(0.5, 0.999))
...
show_fid_mnist_hist(fid_hist, save=True, path='Anime_cDCGAN_results/Anime_cDCGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='Anime_cDCGAN_results/Anime_cDCGAN_FID_hist_victim.png')

#Save results in log for backup
fid_log = "Epoch = " + str(fid_hist['epoch_num']) + "\n\nAnime = " + str(fid_hist['FID_score_mnist'])
