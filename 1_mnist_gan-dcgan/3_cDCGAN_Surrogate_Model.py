        ...
        if (epoch+1)%check_epoch == 0:
            ...
            for label in range(10):
                ...
                for g in range(picture_num_generate):
                    if not os.path.exists('./dc_img/epoch_'+str(epoch+1)+'/'+str(label)):
                        os.mkdir('./dc_img/epoch_'+str(epoch+1)+'/'+str(label))
                    ...
                    save_image(prediction_img.clamp(0,1), './dc_img/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), nrow=1, format='png')
                    if g < picture_num_calculate:
                        copyfile('./dc_img/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), './dc_img/epoch_{}/g_all/fake_images_{}_{}.png'.format(epoch+1,label,g+1))
            print("Image of epoch "+str(epoch+1)+" saved.")

            #Calculate fid scores to MNIST and to victim output and save in dictionary
            fid_mnist = fid_score.calculate_fid_given_paths(['./dc_img/epoch_{}/g_all'.format(epoch+1),
                                                            '../o_img_6000'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
            ...
            fid_victim = fid_score.calculate_fid_given_paths(['./dc_img/epoch_{}/g_all'.format(epoch+1),
                                                            dataset_path+'g_all'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
...
torch.save(G, "MNIST_cDCGAN_results/generator_param.pth")
...
show_fid_mnist_hist(fid_hist, save=True, path='MNIST_cDCGAN_results/MNIST_cDCGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='MNIST_cDCGAN_results/MNIST_cDCGAN_FID_hist_victim.png')
...
with open('MNIST_cDCGAN_results/MNIST_cDCGAN_FID_log.txt','w+') as l_w:
    l_w.write(fid_log)
l_w.close()
