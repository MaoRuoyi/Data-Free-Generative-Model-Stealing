...
Test Using Different Optimizers
...
if not os.path.isdir('MNIST_cDCGAN_results_adab'):
    os.mkdir('MNIST_cDCGAN_results_adab')
if not os.path.isdir('MNIST_cDCGAN_results_adab/Fixed_results'):
    os.mkdir('MNIST_cDCGAN_results_adab/Fixed_results')
if not os.path.isdir('MNIST_cDCGAN_results_adab/Backup'):
    os.mkdir('MNIST_cDCGAN_results_adab/Backup')
if not os.path.isdir('dc_img_adab'):
    os.mkdir('dc_img_adab')
...
#d_optimizer = torch.optim.RMSprop(D.parameters(), lr=0.00002, alpha=0.99)
#g_optimizer = torch.optim.RMSprop(G.parameters(), lr=0.0002, alpha=0.99)
#d_optimizer = torch.optim.SGD(D.parameters(), lr=0.0002, momentum=0.9)
#g_optimizer = torch.optim.SGD(G.parameters(), lr=0.002, momentum=0.9)
#d_optimizer = AdaBelief(D.parameters(), lr=0.00002, eps=1e-16, betas=(0.5,0.999), weight_decay = 0, weight_decouple = True, rectify = False)
#g_optimizer = AdaBelief(G.parameters(), lr=0.0002, eps=1e-16, betas=(0.5,0.999), weight_decay = 0, weight_decouple = True, rectify = False)
...
for epoch in range(train_epoch):
    ...
    with torch.no_grad():
        ...
        fixed_p = 'MNIST_cDCGAN_results_adab/Fixed_results/MNIST_cDCGAN_' + str(epoch + 1) + '.png'
        ...
        if (epoch+1)%check_epoch == 0:
            torch.save(G, 'MNIST_cDCGAN_results_adab/Backup/generator_epoch_{}.pth'.format(epoch+1))
            torch.save(D, 'MNIST_cDCGAN_results_adab/Backup/discriminator_epoch_{}.pth'.format(epoch+1))
            if not os.path.exists('./dc_img_adab/epoch_'+str(epoch+1)):
                os.mkdir('./dc_img_adab/epoch_'+str(epoch+1))
            for label in range(10):
                ...
                for g in range(picture_num_generate):
                    if not os.path.exists('./dc_img_adab/epoch_'+str(epoch+1)+'/'+str(label)):
                        os.mkdir('./dc_img_adab/epoch_'+str(epoch+1)+'/'+str(label))
                    if not os.path.exists('./dc_img_adab/epoch_'+str(epoch+1)+'/g_all'):
                        os.mkdir('./dc_img_adab/epoch_'+str(epoch+1)+'/g_all')
                    ...
                    save_image(prediction_img.clamp(0,1), './dc_img_adab/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), nrow=1, format='png')
                    if g < picture_num_calculate:
                        copyfile('./dc_img_adab/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), './dc_img_adab/epoch_{}/g_all/fake_images_{}_{}.png'.format(epoch+1,label,g+1))
            print("Image of epoch "+str(epoch+1)+" saved.")

            fid_mnist = fid_score.calculate_fid_given_paths(['./dc_img_adab/epoch_{}/g_all'.format(epoch+1),
                                                            '../o_img_6000'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
            ...
            fid_victim = fid_score.calculate_fid_given_paths(['./dc_img_adab/epoch_{}/g_all'.format(epoch+1),
                                                            dataset_path+'g_all'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
...
torch.save(G, "MNIST_cDCGAN_results_adab/generator_param.pth")
torch.save(D, "MNIST_cDCGAN_results_adab/discriminator_param.pth")

show_train_hist(train_hist, save=True, path='MNIST_cDCGAN_results_adab/MNIST_cDCGAN_train_hist.png')
show_fid_mnist_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_adab/MNIST_cDCGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_adab/MNIST_cDCGAN_FID_hist_victim.png')

fid_log = "Epoch = " + str(fid_hist['epoch_num']) + "\n\nMNIST = " + str(fid_hist['FID_score_mnist']) + "\n\nVictim = " + str(fid_hist['FID_score_victim'])
with open('MNIST_cDCGAN_results_adab/MNIST_cDCGAN_FID_log.txt','w+') as l_w:
    l_w.write(fid_log)
l_w.close()
