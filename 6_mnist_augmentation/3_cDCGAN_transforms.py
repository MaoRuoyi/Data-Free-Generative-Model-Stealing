...
if not os.path.isdir('MNIST_cDCGAN_results_transforms'):
    os.mkdir('MNIST_cDCGAN_results_transforms')
if not os.path.isdir('MNIST_cDCGAN_results_transforms/Fixed_results'):
    os.mkdir('MNIST_cDCGAN_results_transforms/Fixed_results')
if not os.path.isdir('MNIST_cDCGAN_results_transforms/Backup'):
    os.mkdir('MNIST_cDCGAN_results_transforms/Backup')
if not os.path.isdir('dc_img_transform'):
    os.mkdir('dc_img_transform')
if not os.path.isdir(dataset_path + 'transforms/'):
    os.mkdir(dataset_path + 'transforms/')
...
class MyDataset(Dataset):
    def __init__(self, txt_path, transform = None, target_transform = None):
        ...
        for line in fh:
            ...
            kernel_size = random.randint(3, 15)  # Random kernel size between 3 and 15
            kernel_size = kernel_size if kernel_size % 2 == 1 else kernel_size + 1 #need to be odd number
            sigma = random.uniform(0.1, 2.0)    #Random sigma value between 0.1 and 2.0
    ...
    def __getitem__(self, index):
        ...
        if self.transform is img_transform:
            img = self.transform(img) 
        elif self.transform is enhanced_transform:
            img = self.complex_transform(img)
            fn_split = re.split('/',fn)
            save_image(img, dataset_path + 'transforms/' + 'trans_' + str(label) + '_' + fn_split[-1], nrow=1, format='png')
        return img, label
    def __len__(self):
        return len(self.imgs)

dataset = MyDataset(txt_path = "./Dataset.txt", transform=img_transform)
enhanced_transform = transforms.ToTensor()
enhanced_dataset = MyDataset(txt_path = "./Dataset.txt", transform=enhanced_transform)
dataset = torch.utils.data.dataset.ConcatDataset([enhanced_dataset, dataset]) #Concatenate dataset with and without further transformations
...
for epoch in range(train_epoch):
    ...
    with torch.no_grad(): 
        ...
        fixed_p = 'MNIST_cDCGAN_results_transforms/Fixed_results/MNIST_cDCGAN_' + str(epoch + 1) + '.png'
        ...
        if (epoch+1)%check_epoch == 0:
            torch.save(G, 'MNIST_cDCGAN_results_transforms/Backup/generator_epoch_{}.pth'.format(epoch+1))
            torch.save(D, 'MNIST_cDCGAN_results_transforms/Backup/discriminator_epoch_{}.pth'.format(epoch+1))
            if not os.path.exists('./dc_img_transform/epoch_'+str(epoch+1)):
                os.mkdir('./dc_img_transform/epoch_'+str(epoch+1))
            for label in range(10):
                ...
                for g in range(picture_num_generate):
                    if not os.path.exists('./dc_img_transform/epoch_'+str(epoch+1)+'/'+str(label)):
                        os.mkdir('./dc_img_transform/epoch_'+str(epoch+1)+'/'+str(label))
                    if not os.path.exists('./dc_img_transform/epoch_'+str(epoch+1)+'/g_all'):
                        os.mkdir('./dc_img_transform/epoch_'+str(epoch+1)+'/g_all')
                    ...
                    save_image(prediction_img.clamp(0,1), './dc_img_transform/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), nrow=1, format='png')
                    if g < picture_num_calculate:
                        copyfile('./dc_img_transform/epoch_{}/{}/fake_images_{}_{}.png'.format(epoch+1,label,label,g+1), './dc_img_transform/epoch_{}/g_all/fake_images_{}_{}.png'.format(epoch+1,label,g+1))
            print("Image of epoch "+str(epoch+1)+" saved.")

            fid_mnist = fid_score.calculate_fid_given_paths(['./dc_img_transform/epoch_{}/g_all'.format(epoch+1),
                                                            '../o_img_6000'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
            ...
            fid_victim = fid_score.calculate_fid_given_paths(['./dc_img_transform/epoch_{}/g_all'.format(epoch+1),
                                                            dataset_path+'g_all'],
                                                            batch_size=1,
                                                            dims = 2048,
                                                            device = device,
...
torch.save(G, "MNIST_cDCGAN_results_transforms/generator_param.pth")
torch.save(D, "MNIST_cDCGAN_results_transforms/discriminator_param.pth")

show_train_hist(train_hist, save=True, path='MNIST_cDCGAN_results_transforms/MNIST_cDCGAN_train_hist.png')
show_fid_mnist_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_transforms/MNIST_cDCGAN_FID_hist_mnist.png')
show_fid_victim_hist(fid_hist, save=True, path='MNIST_cDCGAN_results_transforms/MNIST_cDCGAN_FID_hist_victim.png')

fid_log = "Epoch = " + str(fid_hist['epoch_num']) + "\n\nMNIST = " + str(fid_hist['FID_score_mnist']) + "\n\nVictim = " + str(fid_hist['FID_score_victim'])
with open('MNIST_cDCGAN_results_transforms/MNIST_cDCGAN_FID_log.txt','w+') as l_w:
    l_w.write(fid_log)
l_w.close()
